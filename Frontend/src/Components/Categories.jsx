...
export default function Categories({ categoriesData }) {
   ...
   useEffect(() => {
    // Refresh AOS whenever Categories component renders
    AOS.refresh();
  }, [categoriesData]);
  return (
    <div className=' mx-4 sm:mx-4 md:mx-6 lg:mx-14'>
      <h1 className='text-3xl font-bold mb-8 mt-4 text-[#282828]'>All Categories</h1>
      <div className='grid w-full items-center sm:grid-cols-2 px-4 sm:px-4 md:grid-cols-3 lg:grid-cols-4 lg:px-0 gap-4 my-4'>
        {
            ...
            return (
              <Link to={`/category/${items.id}`} key={items.id}
                ...
                <div className='w-full flex justify-center' data-aos="zoom-in">
                  <img src={items.image_url} alt={items.name} className='max-w-full h-auto' />
                </div>
